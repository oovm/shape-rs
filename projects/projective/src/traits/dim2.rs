    ...
    T: Float,
{
    ...
    /// \end{bmatrix}
    /// ```
    fn transform(&mut self, matrix: &[&T; 9]) -> Self;
    ...
    /// ```
    #[inline(always)]
    fn translate(&mut self, x: &T, y: &T) -> Self {
    ...
    /// ```
    #[inline(always)]
    fn translate_x(&mut self, x: &T) -> Self {
    ...
    /// ```
    #[inline(always)]
    fn translate_y(&mut self, y: &T) -> Self {
    ...
    #[inline(always)]
    #[rustfmt::skip]
    fn rotate(&mut self, angle: &T) -> Self {
        let cos = angle.cos();
        let sin = angle.sin();
        self.transform(&[
            &cos,       &sin,       &T::zero(),
    ...
    /// ```
    #[inline(always)]
    fn rotate_point(&mut self, point: &[&T; 2], angle: &T) -> Self {
    ...
    #[inline(always)]
    #[rustfmt::skip]
    fn scale(&mut self, x: &T, y: &T) -> Self {
        self.transform(&[
            x,          &T::zero(), &T::zero(),
            &T::zero(), y,          &T::zero(),
            &T::zero(), &T::zero(), &T::one(),
    ...
    /// ```
    #[inline(always)]
    fn scale_y(&mut self, y: &T) -> Self {
        self.scale(&T::one(), y)
    }
    /// Transform by skew $x, y$.
    #[inline(always)]
    #[rustfmt::skip]
    fn reflect(&mut self, x: &T, y: &T) -> Self {
        self.transform(&[
            &T::one(),  &T::zero(), &T::zero(),
            &T::zero(), &T::one(),  &T::zero(),
            x,          y,          &T::one(),
